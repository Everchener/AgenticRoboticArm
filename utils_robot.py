...
def move_to_top_view():
    print('移动至俯视姿态')
    mc.send_angles([-89.03, 8.34, -63.63, -32.08, 5.27, 41.39], 10)
    time.sleep(3)
...
def gripper_move(mc, XY_START=[230, -50], HEIGHT_START=90, XY_END=[100, 220], HEIGHT_END=100, HEIGHT_SAFE=220):
    '''
    用夹爪，将物体从起点抓取移动至终点
    '''
    mc.set_fresh_mode(0)
...
def gripper_lift(mc, XY_START=[230, -50], HEIGHT_START=90, HEIGHT_SAFE=220):
    '''
    用夹爪抓取物体并抬起
    '''
    mc.set_fresh_mode(0)
...
def gripper_drop(XY_END=None, HEIGHT_END=100, HEIGHT_SAFE=220):
    '''
    把手上的物体放到指定位置
    '''
    if XY_END is None:
        XY_END = [mc.get_coords()[0], mc.get_coords()[1]]
