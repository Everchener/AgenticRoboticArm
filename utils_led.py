...
from utils_llm import llm_qianfan
...
# 系统提示词
SYS_PROMPT = '''我即将说的这句话中包含一个目标物体，帮我把这个物体的一种可能的颜色，以0-255的RGB像素值形式返回给我，整理成元组格式，例如(255, 30, 60)，直接回复元组本身，以括号开头，括号内只包含RGB像素值的数字，不包含任何中文，前后都不要包含中文，只要数字，不要回复任何中文内容，下面是这句话：'''
...
def llm_led(PROMPT_LED='帮我把LED灯的颜色改为贝加尔湖的颜色'):
    ...
    参数:
        PROMPT_LED: 描述期望LED灯颜色的文本
    
    注意: 需要连接MyCobot机械臂，并配置好LED控制
    '''
