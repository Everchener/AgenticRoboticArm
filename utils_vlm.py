...
# 默认API配置（需要用户修改）
YI_VISION_API_KEY = "YOUR_API_KEY"  # 替换为您的零一万物API Key
YI_VISION_API_BASE = "https://api.lingyiwanwu.com/v1"


def yi_vision_api(PROMPT='帮我把红色方块放在小猪佩奇上', 
    ...
    返回:
        解析后的结果字典，包含start, start_xyxy, end, end_xyxy
    '''
    if api_key is None:
        api_key = YI_VISION_API_KEY
    
    client = OpenAI(
        api_key=api_key,
        base_url=YI_VISION_API_BASE
    )
...
def post_processing_viz(result, img_path, check=False):
    ...
    返回:
        起点和终点的中心像素坐标 (START_X_CENTER, START_Y_CENTER, END_X_CENTER, END_Y_CENTER)
    '''
    # 后处理
